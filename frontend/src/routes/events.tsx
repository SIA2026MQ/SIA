  ...
  const handleViewDetails = (webinar: any) => {
    if (!dbUser) {
      setPopup({
        ...
        message:
          "Please sign in or create an account to view webinar details and join sessions.",
        type: "alert",
            ...
            >
              <div
                className={`absolute top-0 left-0 w-full h-1.5 ${
                  event.category === "QnA" ? "bg-blue-500" : "bg-orange-500"
                }`}
              ></div>
              <div
                className={`w-16 h-16 rounded-2xl flex flex-col items-center justify-center text-white font-bold mb-5 shadow-sm ${
                  event.category === "QnA"
...
function RetreatsTabContent({
  ...
  return (
    <>
              ...
              <div className="p-6 flex flex-col flex-1">
                ...
                <div className="space-y-2 mb-6 text-xs font-semibold text-gray-700">
                  <p className="flex items-center gap-2">
                    <MapPin className="h-3.5 w-3.5 text-[#600694]" />{" "}
                    {retreat.location}
                  </p>
                  <p className="flex items-center gap-2">
                    <Calendar className="h-3.5 w-3.5 text-[#600694]" />{" "}
                ...
                <button
                  ...
                  className="w-full bg-[#600694]/10 text-[#600694] py-2.5 rounded-full font-bold hover:bg-[#600694] hover:text-white transition-colors"
                >
                  View Details & Apply
                </button>
              </div>
      ...
      {/* Full Screen Split Modal for Retreats */}
      {selectedRetreat && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 md:p-8 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-5xl max-h-[90vh] flex flex-col md:flex-row overflow-hidden shadow-2xl relative animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="md:w-1/2 bg-gray-50 p-8 md:p-12 overflow-y-auto custom-scrollbar">
              ...
              <h2 className="text-3xl font-display font-bold text-gray-900 mt-4 mb-4">
              ...
              <div className="p-8 md:p-12 flex-1 flex flex-col">
                ...
                <div className="space-y-4 flex-1">
                  <label className="block">
                    ...
                    <input
                      className="w-full p-3 mt-1 border rounded-xl focus:outline-none focus:border-[#600694]"
                ...
                <div className="mt-8 pt-6 border-t flex items-center justify-between">
                  <div className="flex items-center gap-1.5 text-xs font-semibold text-gray-500 italic">
                    <Lock className="h-3.5 w-3.5" /> Price hidden until
                    approved
                  </div>
                  <button
                    onClick={handleApply}
                    className="px-8 py-3 rounded-full font-bold bg-[#600694] text-white shadow-md hover:bg-[#4a0473] transition-colors"
                  >
                    Submit Details
                  </button>
                </div>
    ...
    <div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 mt-2">
        ...
        {webinars.map((webinar: any) => (
          <div
            key={webinar.id}
            className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm flex flex-col hover:shadow-md transition-shadow group"
          >
