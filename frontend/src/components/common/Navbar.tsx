  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${needsSolidBg ? "bg-background/95 backdrop-blur-md border-b border-border/40 shadow-sm h-[72px]" : "bg-transparent border-transparent h-[88px]"}`}>
      <div className="sia-container flex h-full items-center justify-between gap-3 md:gap-6 lg:gap-10 px-4 sm:px-6">
        ...
        {/* 🚀 UPDATED: Desktop Nav - stretch links with justify-between and larger text */}
          ...
          {navLinks.map((link) => {
            ...
            if (hasDropdown) {
              ...
              return (
                ...
                >
                  <div
                    className={`flex items-center gap-1 text-base font-semibold transition-colors hover:text-primary cursor-pointer ${
                      needsSolidBg ? 'text-foreground/90' : 'text-white drop-shadow-md'
                    }`}
                  >
                    {hasDropdown.to ? (
                      <Link to={hasDropdown.to} className="flex items-center gap-1">
                        {link.label}
                        {link.label === "Events" && hasEventNotification && (
                  ...
                  >
                    {hasDropdown.children.map((child, childIdx) => {
                      const showDot =
                        (child.label === "Webinars" && notifications.webinars) ||
                      ...
                      return (
                        <Link
                          ...
                          className="flex items-center justify-between rounded-lg px-3 py-2 text-base font-medium text-foreground/85 transition hover:bg-accent hover:text-primary"
                          onBlur={() => {
                  ...
                  {active && (
                    <span
                      className={`absolute bottom-2 left-0 h-0.5 w-full transition-colors ${
                        needsSolidBg ? 'bg-primary' : 'bg-white'
                      }`}
                    />
                  )}
                </div>
            ...
            return (
              <NavLink
                ...
                to={link.to}
                className={({ isActive }) =>
                  `relative py-4 px-1 text-base font-semibold transition-colors hover:text-primary ${
                    needsSolidBg ? 'text-foreground/90' : 'text-white drop-shadow-md'
                  }`
                }
        ...
        <div className="flex items-center gap-2 md:gap-3">
            ...
            {!loading && dbUser ? (
                      ...
                      <div className="p-4 flex items-center gap-3 bg-gray-50/50">
                        ...
                        <div className="flex flex-col min-w-0">
                          <div className="flex items-center gap-2">
                            <span className={`font-bold text-sm truncate ${isBlocked ? 'text-red-600 line-through' : 'text-foreground'}`}>
                              {dbUser.name}
                            </span>
                            {isBlocked ? (
              ...
              <Link
                ...
                className={`h-11 items-center gap-2 rounded-full border px-5 text-sm font-semibold transition inline-flex ${
                  needsSolidBg ? 'border-primary/30 text-primary hover:bg-accent' : 'border-white/50 text-white bg-white/10 backdrop-blur-sm hover:bg-white hover:text-primary'
                }`}
              >
          ...
          <Link
            ...
            className={`relative inline-flex min-h-11 min-w-11 items-center justify-center rounded-full bg-white shadow-md transition-transform ${
              isBlocked ? 'text-gray-300 opacity-50 cursor-not-allowed' : 'text-primary hover:scale-105'
            }`}
          >
            ...
            <div className="mx-auto w-full max-w-lg space-y-4">
              {dbUser && liveSession && !isBlocked && (
                <button
                  ...
                  className={`block w-full rounded-xl border px-4 py-3 font-display text-xl text-center animate-pulse ${
                    liveSession.sessionType === 'QnA' ? 'border-blue-500 bg-blue-50 text-blue-600' : 'border-red-500 bg-red-50 text-red-600'
                  }`}
                >
              ...
              <Link
                to="/"
                className="block rounded-xl border border-border bg-card px-4 py-3 font-display text-2xl text-primary"
                    ...
                    <div className="flex items-center justify-between px-4 py-3">
                      {hasDropdown.to ? (
                        <Link
                          to={hasDropdown.to}
                          onClick={() => setOpen(false)}
                          className="flex items-center gap-2 font-display text-2xl text-primary"
                        >
                          {link.label}
                          {link.label === "Events" && hasEventNotification && (
                            <span className="h-2 w-2 rounded-full bg-red-500 animate-pulse shadow-[0_0_8px_rgba(239,68,68,0.8)]" />
                      ...
                      <button
                        ...
                        onClick={() => setOpenMobileGroup(expanded ? null : hasDropdown.label)}
                      >
            ...
            <div className="mx-auto mt-7 flex flex-col w-full max-w-lg gap-4 border-t border-gray-100 pt-6">
              {!loading && dbUser ? (
                <div className="w-full flex flex-col gap-3">
                  <Link
                    to={isAdmin ? "/admin" : "/my-learning"}
                    className="sia-button-primary text-center py-3 rounded-full font-bold text-sm block"
                    onClick={() => setOpen(false)}
                  >
                  ...
                  <button
                    ...
                    className="w-full py-3 border border-red-200 text-red-600 rounded-full font-bold text-sm bg-red-50/50 flex items-center justify-center gap-2"
                  >
                ...
                <div className="flex items-center gap-3 w-full">
                  <Link
                    to="/login"
                    className="sia-button-outline flex-1 text-center py-3 rounded-full font-bold text-sm"
                    onClick={() => setOpen(false)}
                  >
                    Login
                  </Link>
