...
export function WebinarGrid({ webinars, setWebinars, loading, subscription, setSubscription, isLoggedIn }: WebinarGridProps) {
  ...
  const handleJoinClick = async (webinar: any) => {
    ...
    // 2. If already unlocked, just open the link
    ...
    // 5. Confirm and process redemption
    setPopup({
      isOpen: true,
  ...
  // 🚨 NEW: Sort webinars so the newest ones appear first
  const sortedWebinars = [...webinars].sort((a, b) => {
    // Fallback through common date fields to find the valid timestamp
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 md:p-8">
              <h3 className="text-xl sm:text-2xl font-display font-bold text-gray-900 mb-2 sm:mb-3">{popup.title}</h3>
              <p className="text-sm sm:text-base text-gray-600 mb-6 sm:mb-8 leading-relaxed">{popup.message}</p>

              <div className="flex gap-3 justify-end mt-auto">
                {popup.type === "confirm" && (
                  <button
                    onClick={popup.onCancel}
                    className="px-4 sm:px-6 py-2 sm:py-3 text-sm sm:text-base text-gray-600 hover:bg-gray-100 rounded-lg sm:rounded-xl font-bold transition-colors"
                  >
                ...
                <button
                  ...
                  className="px-4 sm:px-6 py-2 sm:py-3 text-sm sm:text-base bg-[#600694] hover:bg-[#4a0473] text-white rounded-lg sm:rounded-xl font-bold transition-colors shadow-md"
                >
      ...
      {/* WEBINAR GRID - 2 columns on mobile, 3 on desktop */}
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6 mt-2">
        {sortedWebinars.length === 0 && (
          <div className="col-span-2 lg:col-span-3 text-center p-6 sm:p-10 bg-white rounded-2xl sm:rounded-3xl border border-gray-100">
            <p className="text-xs sm:text-sm text-muted-foreground">No upcoming webinars scheduled right now.</p>
          </div>
        )}

        {/* 🚨 Use sortedWebinars instead of webinars here */}
          ...
          <div className="bg-white rounded-none sm:rounded-2xl lg:rounded-3xl w-full max-w-5xl h-[100dvh] sm:h-[95vh] md:max-h-[90vh] flex flex-col shadow-2xl relative animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row w-full h-full overflow-y-auto md:overflow-hidden relative custom-scrollbar">
              ...
              <div className="w-full md:w-1/2 bg-gray-50 p-6 md:p-12 order-2 md:order-1 h-max md:h-full md:overflow-y-auto custom-scrollbar pb-16 md:pb-12">
                ...
                <div className="flex flex-wrap gap-4 mb-8 text-sm font-semibold text-gray-700 bg-white p-4 rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm">
                ...
                <div className="p-6 md:p-12 flex-1 flex flex-col pb-12">
                  ...
                  <div className="mt-auto pt-6 border-t border-gray-100">
                    <button
                      ...
                      className={`w-full py-3.5 sm:py-4 rounded-full font-bold flex items-center justify-center gap-2 transition-all transform hover:-translate-y-0.5 shadow-md hover:shadow-lg ${selectedWebinar.hasAccess
